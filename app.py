...
def index():
    ...
    purchases = db.execute("SELECT symbol, SUM(amount*action) as amount FROM purchases where user_id = ? GROUP BY symbol", session["user_id"])
    sum = 0
    for p in purchases:
        ...
        sum += p["total"]
    return render_template("index.html", purchases=purchases, cash=fundsavailable, total=sum+fundsavailable)
...
def history():
    ...
    purchases = db.execute("SELECT symbol, name, price, amount, action, timestamp FROM purchases where user_id = ?", session["user_id"])
    for p in purchases:
...
def sell():
    ...
    if request.method == "POST":
        ...
        available = db.execute("SELECT SUM(amount * action) available FROM purchases where user_id = ? and symbol = ?", session["user_id"], symbol)
        if int(shares) > available[0]["available"]:
            return apology("Not enough shares to sell")

        # Add sell transaxction to database
        db.execute("INSERT INTO purchases (user_id, symbol, name, price, amount, timestamp, action) VALUES (?, ?, ?, ?, ?, ?, ?)", session["user_id"], quote["symbol"], quote["name"], quote["price"], shares, datetime.datetime.now(), -1)
        db.execute("UPDATE users SET cash = ? WHERE id = ?", fundsavailable + (quote["price"] * int(shares)), session["user_id"])
